...
# Pools - we will look at them from a few angles, available members, pick count, etc
tmsh -q -c "cd /; list ltm pool recursive one-line" | awk '{print "/"$3}' | while read pool; do if [ "$(tmsh show ltm pool $pool field-fmt raw | grep 'clientside.bits-in' | awk '{print $2}')" -eq 0 ]; then echo $pool; fi; done